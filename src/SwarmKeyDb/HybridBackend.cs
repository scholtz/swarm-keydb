...
public sealed class HybridBackend : IKeyValueStore, IBackendMetadataProvider
{
    private readonly SwarmKeyValueStore _inner;

    public HybridBackend(ISwarmClient swarmClient, ISwarmClient ipfsClient, IKeyIndex index, IntegrityOptions? integrityOptions = null)
    {
        _inner = new SwarmKeyValueStore(new HybridSwarmClient(swarmClient, ipfsClient), index, integrityOptions);
    }
