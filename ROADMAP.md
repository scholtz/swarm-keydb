...
- [ ] Add multi-node integration tests and production telemetry for cache drift, verification failures, synchronization lag, and forced resync counts so operators can prove consistency during failover and rolling deployments. (65%: added forced-resync telemetry counters `swarmkeydb_resync_partial_total`, `swarmkeydb_resync_full_total`, `swarmkeydb_resync_duration_seconds`, and `swarmkeydb_resync_keys_replayed_total`, plus coordinator/endpoint/command coverage; remaining work is multi-node docker failover scenarios and drift-focused production-grade integration suites)
