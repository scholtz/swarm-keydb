...
var tests = new (string Name, Func<Task> Test)[]
{
    ...
    ("pubsub subcommands channels numsub numpat", PubSubSubCommandsAsync),
    ("pubsub unsubscribe reduces count and removes from active channels", PubSubUnsubscribeReducesCountAsync),
    ...
    ("eval executes lua script and returns integer", EvalReturnsIntegerAsync),
    ("eval executes lua script and returns string", EvalReturnsStringAsync),
    ("eval executes lua script and returns table as array", EvalReturnsTableAsArrayAsync),
    ("eval executes lua script and returns nil", EvalReturnsNilAsync),
    ("eval keys and argv are accessible", EvalKeysAndArgvAreAccessibleAsync),
    ("eval redis call dispatches to command processor", EvalRedisCallDispatchesAsync),
    ("eval redis pcall catches errors without abort", EvalRedisPCallCatchesErrorsAsync),
    ("eval arity error returned for missing arguments", EvalArityErrorAsync),
    ("eval numkeys out of range returns error", EvalNumKeysOutOfRangeAsync),
    ("evalsha returns noscript on cache miss", EvalShaNoscriptOnCacheMissAsync),
    ("evalsha executes cached script after script load", EvalShaAfterScriptLoadAsync),
    ...
    ("eval sandbox blocks os access", EvalSandboxBlocksOsAsync),
    ("eval full round trip set get via redis call", EvalFullRoundTripAsync),
    ("eval redlock style atomic lock script", EvalRedlockStyleLockAsync),
    ("prometheus metrics expose script telemetry", PrometheusMetricsExposeScriptTelemetryAsync),
};
...
static RedisCommandProcessor CreatePubSubProcessor(PubSubManager? manager = null)
{
...
static async Task PubSubSubscribeAndPublishSingleNodeAsync()
{
...
static async Task PubSubPatternSubscribeReceivesMatchingChannelMessagesAsync()
{
    var manager = new PubSubManager();
    var processor = CreatePubSubProcessor(manager);
...
static async Task PubSubMultiSubscriberFanOutDeliversToAllAsync()
{
    var manager = new PubSubManager();
    var processor = CreatePubSubProcessor(manager);
...
static async Task PubSubSubCommandsAsync()
{
    var manager = new PubSubManager();
    var processor = CreatePubSubProcessor(manager);
...
static async Task PubSubUnsubscribeReducesCountAsync()
{
    var manager = new PubSubManager();
    var processor = CreatePubSubProcessor(manager);
...
static async Task PubSubSlowSubscriberDoesNotBlockOtherSubscribersAsync()
{
    var manager = new PubSubManager();
    var processor = CreatePubSubProcessor(manager);
...
static async Task PubSubCrossNodeDeliveryViaInMemoryBusAsync()
{
    var bus = new InMemoryCacheSyncBus();
...
static async Task PubSubGlobPatternMatchingAsync()
{
...
static (Stream Input, MemoryStream Output) CreatePipe()
{
...
static async Task WriteRespCommandAsync(Stream stream, params string[] parts)
{
    var builder = new StringBuilder();
    builder.Append('*').Append(parts.Length).Append("\r\n");
    foreach (var part in parts)
...
static RespValue BuildRespCommand(params string[] parts)
{
    var items = parts.Select(static p => RespValue.BulkString(p)).ToArray();
    return RespValue.Array(items);
}

static string ReadAllBytes(MemoryStream stream)
{
...
/// <summary>
/// A stream that blocks on Read until data is written to it.
/// Supports concurrent reads and writes for the pub/sub test scenarios.
/// </summary>


// ─── Scripting tests ──────────────────────────────────────────────────────────

static async Task EvalReturnsIntegerAsync()
{
    var processor = CreateProcessor();
...
static async Task EvalReturnsStringAsync()
{
    var processor = CreateProcessor();
...
static async Task EvalReturnsTableAsArrayAsync()
{
    var processor = CreateProcessor();
...
static async Task EvalReturnsNilAsync()
{
    var processor = CreateProcessor();
...
static async Task EvalKeysAndArgvAreAccessibleAsync()
{
    var processor = CreateProcessor();
...
static async Task EvalRedisCallDispatchesAsync()
{
    var processor = CreateProcessor();
...
static async Task EvalRedisPCallCatchesErrorsAsync()
{
    var processor = CreateProcessor();
...
static async Task EvalArityErrorAsync()
{
    var processor = CreateProcessor();
...
static async Task EvalNumKeysOutOfRangeAsync()
{
    var processor = CreateProcessor();
...
static async Task EvalShaNoscriptOnCacheMissAsync()
{
    var processor = CreateProcessor();
    var resp = await ExecuteAsync(processor,
...
static async Task EvalShaAfterScriptLoadAsync()
{
    var processor = CreateProcessor();
...
static async Task ScriptLoadReturnsSha1Async()
{
    var processor = CreateProcessor();
    var script = "return 1";
...
static async Task ScriptExistsAsync()
{
    var processor = CreateProcessor();
...
static async Task ScriptFlushAsync()
{
    var processor = CreateProcessor();
...
static async Task EvalTimeoutReturnsBusyAsync()
{
...
static async Task EvalSandboxBlocksIoAsync()
{
    var processor = CreateProcessor();
...
static async Task EvalSandboxBlocksOsAsync()
{
    var processor = CreateProcessor();
...
static async Task EvalFullRoundTripAsync()
{
    var processor = CreateProcessor();
...
static async Task EvalRedlockStyleLockAsync()
{
